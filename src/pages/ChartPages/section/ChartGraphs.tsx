...
const ChartGraphs = () => {
  ...
  const fromRef = useRef<Time | null>(null)
  const toRef = useRef<Time | null>(null)
  ...
  chartObjectRef.current
    ...
    .subscribeVisibleTimeRangeChange((timeRange) => {
      if (!timeRange) return
      fromRef.current = timeRange.from ?? null
      toRef.current = timeRange.to ?? null
      ...
      if (firstVisibleBar < 0) {
        ...
        getCandleHistory(
          ...
          typeof fromRef.current === 'number' ? fromRef.current : undefined,
          typeof toRef.current === 'number' ? toRef.current : undefined
        )
      }
    })
