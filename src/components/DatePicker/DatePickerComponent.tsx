...
const DatePickerComponent = (props: DatePickerProps) => {
  ...
  return (
    <div
      className={`pl-4 pr-10 relative bg-landing-page-trading-rules-para-text rounded-lg h-10 ${className}`}
    >
      <DatePicker
        selectsRange
...
viewBox="0 0 16 16" width="20">
      ...
      {isDropDownType && (
        <ImageComponent
          ...
          onPressItem={() => setIsOpen((prev) => !prev)}
          className={`w-3 cursor-pointer absolute right-3 top-1/2 -translate-y-1/2 transition-all duration-200 ease-linear ${
            isOpen ? 'rotate-180' : ''
          }`}
        />
      )}
